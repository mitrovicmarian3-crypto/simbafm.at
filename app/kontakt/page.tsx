...
// Additional locations requested
const additionalLocations = [
  "Schwechat", "Gumpoldskirchen", "Baden", "Wiener Neudorf", 
  "Strebersdorf", "Hagenbrunn", "Kottingbrunn"
];
    ...
    <div className="bg-[#f8fafc] min-h-screen pt-24 pb-0">
      ...
      <section className="py-12 bg-white">
        <div className="max-w-[800px] mx-auto text-center px-6">
          <h2 className="text-2xl sm:text-3xl font-extrabold text-[#003067] leading-tight tracking-tight font-sans">
            Wir sind für Sie da
          </h2>
          <p className="text-slate-600 text-sm sm:text-base leading-relaxed mt-4 font-medium">
            Egal welches Anliegen Sie haben – ob eine regelmäßige Unterhaltsreinigung, eine einmalige Sonderreinigung oder Fragen zu unseren Tarifen: Sie können uns jederzeit für jedes Anliegen kontaktieren. Wir beraten Sie meisterhaft, unverbindlich und erstellen Ihnen zeitnah ein passgenaues Angebot.
          </p>
        </div>
      </section>
        ...
        <div className="max-w-[1200px] mx-auto px-6">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-500 text-sm mt-3 max-w-xl mx-auto font-medium">
              Für unsere gewerblichen und privaten Kunden sind wir flexibel in ganz Wien sowie ausgewählten Umlandgemeinden im Einsatz.
            </p>
          </div>

          {/* Vienna districts card grid */}
          <div className="mb-10">
            <h3 className="text-sm font-bold text-[#003067]/80 uppercase tracking-wider mb-6 text-center lg:text-left font-sans">
              Gemeindebezirke Wien (01. - 23. Bezirk)
            </h3>
            <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-12 gap-3">
              {districts.map((district) => (
                <div 
                  key={district} 
                  className="bg-white border border-slate-200/70 py-3.5 px-2 rounded-xs text-center hover:border-[#0052b4] hover:shadow-3xs transition-all duration-300"
                >
                  <span className="block font-black text-[#003067] text-sm md:text-base font-sans">{district}</span>
                  <span className="block text-[9px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">Wien</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <h3 className="text-sm font-bold text-[#003067]/80 uppercase tracking-wider mb-6 text-center lg:text-left font-sans">
              Weitere Einsatzgebiete in Niederösterreich
            </h3>
            <div className="flex flex-wrap justify-center lg:justify-start gap-3">
              {additionalLocations.map((loc) => (
                <div 
                  key={loc} 
                  className="bg-white border border-slate-200/70 py-3 px-6 rounded-xs hover:border-[#0052b4] hover:shadow-3xs transition-all duration-300 flex items-center justify-center"
                >
                  <span className="font-extrabold text-[#003067] text-sm font-sans">{loc}</span>
                </div>
              ))}
            </div>
          </div>
